...
class RRT_star(object):
    ...
    def __init__(self, obstacle_x, obstacle_y, grid_size, robot_radius, max_point_num=200, max_sample_times=50000,
                 step_size=0.5, prob_goal=0.1, search_radius=2, goal_threshold=1):
        self.obs_tree = KDTree(np.vstack((obstacle_x, obstacle_y)).T)   # Obstacle KD Tree
        self.avoid_dist = grid_size
        self.robot_radius = robot_radius / 2
    ...
    def plan(self, start_x, start_y, goal_x, goal_y):
        ...
        while new_index < self.max_point_num + 1:
            ...
            if new_x and new_y:
                ...
                rrt_x.append(new_x)
                rrt_y.append(new_y)
                road_map = self.rewire(rrt_x, rrt_y, new_x, new_y, road_map, new_index)
                # print(road_map)
    ...
    def rewire(self, rrt_x, rrt_y, new_x, new_y, road_map, new_index):
        ...
        for i in range(new_index):
            # search for potential father node
            if self.cal_euler_dst(rrt_x[i], rrt_y[i], new_x, new_y) < self.search_radius:
                ...
                if new_cost < current_cost and not self.check_obs(rrt_x[i], rrt_y[i], new_x, new_y):
...
if __name__ == "__main__":
    ...
    planner = RRT_star(ox, oy, grid_size=0.3, robot_radius=0.8, max_point_num=100, max_sample_times=500, prob_goal=0.5)
